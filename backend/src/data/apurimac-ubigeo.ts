export type UbigeoReference = {
  id: string;
  codigo_ubigeo: string;
  nombre: string;
};

export const APURIMAC_DEPARTMENT = {
...
const provinceNames: Array<[string, string]> = [
  ['0301', 'Abancay'], ['0302', 'Andahuaylas'], ['0303', 'Antabamba'],
  ['0304', 'Aymaraes'], ['0305', 'Cotabambas'], ['0306', 'Chincheros'], ['0307', 'Grau'],
];
...
export const APURIMAC_PROVINCES: UbigeoReference[] = provinceNames.map(([code, nombre]) => ({
  id: provinceId(code), codigo_ubigeo: code, nombre,
}));

export const getApurimacDistricts = (selectedProvinceId: string): UbigeoReference[] => {
  const provinceCode = selectedProvinceId.slice(-4);
  return (districtNames[provinceCode] || []).map((nombre, index) => {
    const code = `${provinceCode}${String(index + 1).padStart(2, '0')}`;
    return { id: districtId(code), codigo_ubigeo: code, nombre };
  });
};
